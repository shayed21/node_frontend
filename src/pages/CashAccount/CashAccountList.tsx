...
export const CashAccountList: React.FC = () => {
	...
	const deleteCashAccount = async (id: number) => {
		try {
			...
			setCashAccount((prevCashAccount) => prevCashAccount.filter((cashAc) => cashAc._id !== id));
		} catch (error) {
	...
	const filteredCashAccount = cashAccount.filter((cashAc) => {
		...
		const matchesCategory =
			selectedCategory === 'all' || cashAc.status?.toLowerCase() === selectedCategory.toLowerCase();
		...
		<div className="space-y-6">
			...
			>
				<div className="flex items-center space-x-4">
					...
					<div>
						<h1 className="text-3xl font-bold text-gray-900 dark:text-white">Cash Account</h1>
						<p className="text-gray-600 dark:text-gray-400">Manage your cash account List</p>
					</div>
				</div>
				<button
								...
								>
									...
									<td className="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
										<div className="flex items-center justify-end space-x-2">
											...
											<button
												onClick={() => navigate(`/cashAccount/${cashAc?._id}/edit`)}
