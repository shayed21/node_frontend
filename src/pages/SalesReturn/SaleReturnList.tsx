...
export const SaleReturnList: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const pendingList = returns.filter((r) => r.status.toLowerCase() === "pending");
    ...
    return [
      { label: "Total Returns", value: currency(total), change: `${returns.length} returns`, color: "text-blue-600" },
      { label: "Pending", value: currency(pendingTotal), change: `${pendingList.length} returns`, color: "text-yellow-600" },
      { label: "Paid", value: currency(paidTotal), change: `${paidList.length} returns`, color: "text-green-600" },
    ];
  }, [returns]);
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Sales Returns</h1>
          <p className="text-gray-600 dark:text-gray-400">Manage sales return invoices</p>
        </div>
        <button
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredReturns.map((r, index) => (
                  ...
                  >
                    <td className="px-6 py-4 whitespace-nowrap">{r.returnNo}</td>
                    <td className="px-6 py-4 whitespace-nowrap">{r.customerName}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {r.returnDate ? new Date(r.returnDate).toLocaleDateString() : "—"}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">${r.totalAmount.toFixed(2)}</td>
                    <td className="px-6 py-4 whitespace-nowrap">${r.dueamount.toFixed(2)}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
